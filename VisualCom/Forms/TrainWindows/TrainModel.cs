...
namespace VisualCom.Forms.Editor
{
    public partial class TrainModel : Form
    {
        ...
        {
            foreach (var path in Directory.GetDirectories(Configuration.ProjectModels))
        ...
        private void GetModels()
        {
            try
            {
                var modelFiles = Directory.GetFiles(Path.Join(".", "models"));
            }
            catch
            {
                Directory.CreateDirectory(Path.Join(".", "models"));
            }

            foreach (string model in Directory.GetFiles(Path.Join(".", "models")))
            {
                string name = Path.GetFileName(Path.GetFileName(model));
                switch (name)
                {
                    case "yolo26n.pt":
        ...
        private async void InitTrainModel(object sender, EventArgs e)
        {
            ...
            Process.Start("explorer", Path.Join(Configuration.ProjectModels, Version));
